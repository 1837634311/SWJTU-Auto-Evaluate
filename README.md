# 🎓 西南交大教务系统一键评价助手 (SWJTU Evaluation Auto-Filler)

## 📖 简介 (Introduction)

这是一个用于 **西南交通大学教务系统 (VATUU)** 的油猴 (Tampermonkey) 脚本，帮助同学快速完成繁琐的课程评价。

不同于其他填充脚本，本脚本安装后，能够一键全自动进行评价。

## ✨ 核心功能
*   **🚀 极速全自动**：一键启动，自动遍历所有未评课程。
*   **🐛 智能绕过限制**：利用系统“提交失败-返回-重试”的机制，绕过系统的最小停留时间检测。
*   **📝 自动好评**：默认全选“A”（5.0分），填充正向评语。
*   **✅ 自动跳转**：评价完成后，自动查看成绩。
*   **🛡️ 安全稳定**：纯本地运行，不上传任何用户数据，也不需要输入账号密码；提供“仅填写当前页”的手动模式。
...
### 3. 开始使用
1.  登录教务系统，进入 **“教学评价” -> “课程评价”** 列表页。
2.  你会看到页面右上角出现一个 **悬浮面板**。
3.  点击 **“🚀 开始全自动评价”**。
4.  喝杯水，脚本会自动处理所有课程。完成后会自动跳转到成绩查询页面。
